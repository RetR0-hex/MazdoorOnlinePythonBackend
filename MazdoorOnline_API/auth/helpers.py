...
from MazdoorOnline_API.models import TokenBlocklist, User, LaborDetail, Reviews
...
def create_labor_detail(user: User, category_id: int):
    detail = LaborDetail(labor_id=user.id, category_id=category_id)
    review = Reviews(labor_id=user.id)
    db.session.add(review)
    db.session.commit()
    db.session.flush()

    review = Reviews.query.filter_by(labor_id=user.id).first()

    detail.reviews_id = review.id
